...
gui.add(unrealBloomPass, "radius").min(0).max(10).name("radius");
gui.add(unrealBloomPass, "threshold").min(0).max(1).name("threshold");
gui.add(unrealBloomPass, "strength").min(0).max(10).name("strength");
...
console.log(displacementPass.material.uniforms.uNormalMap.value);
effectComposer.addPass(displacementPass);

// SMAA Pass(antialiase)
if (renderer.getPixelRatio() === 1 && !renderer.capabilities.isWebGL2) {
  const smaaPass = new SMAAPass();
  effectComposer.addPass(smaaPass);
}

//fix the shader darkness
